...
int RAPCStrategy::getEvenBet() const {
    return 1;
}
...
bool RAPCStrategy::shouldAcceptInsurance() const{
    ...
    // 2-deck 65% pen: TC crossover = 8.0, 6-deck 80% pen: TC crossover = 11.0
    const float insuranceThreshold = useSixDeck ? 11.0f : 8.0f;
...
Action RAPCStrategy::shouldDeviatefromHard(int playerTotal, Rank dealerUpcard, float trueCount){
    ...
    switch (playerTotal) {
        case 16:
            // 2-deck 65% pen: 16v10 Stand TC >= 3.0, 6-deck 80% pen: TC >= 1.0
            if (dealerValue == 10 && trueCount >= (useSixDeck ? 1.0f : 3.0f)) {
                return Action::Stand;
            }
            break;
            
        case 15: 
            // 2-deck 65% pen: 15v10 Stand TC >= 9.5, 6-deck 80% pen: TC >= 11.0
            if (dealerValue == 10 && trueCount >= (useSixDeck ? 11.0f : 9.5f)) {
                return Action::Stand;
            }
            break;

        case 13:
            // 2-deck 65% pen: 13v2 Stand TC >= -1.5, 6-deck 80% pen: TC >= -2.5
            // 2-deck 65% pen: 13v3 Stand TC >= -4.5, 6-deck 80% pen: TC >= -6.5
            if (dealerValue == 2 && trueCount >= (useSixDeck ? -2.5f : -1.5f)) { 
                return Action::Stand;
            }
            if (dealerValue == 3 && trueCount >= (useSixDeck ? -6.5f : -4.5f)) { 
                return Action::Stand;
            }
            break;
            
        case 12:
            // 2-deck 65% pen: 12v3 Stand TC >= 6.5, 6-deck 80% pen: TC >= 5.0
            if (dealerValue == 3 && trueCount >= (useSixDeck ? 5.0f : 6.5f)) {
                return Action::Stand;
            }
            // 2-deck 65% pen: 12v2 Stand TC >= 11.5, 6-deck 80% pen: TC >= 10.0
            if (dealerValue == 2 && trueCount >= (useSixDeck ? 10.0f : 11.5f)) {
                return Action::Stand;
            }
            break;
        ...
        case 10:
            // 2-deck 65% pen: 10v10 Double TC >= 8.5, 6-deck 80% pen: TC >= 9.5
            if (dealerValue == 10 && trueCount >= (useSixDeck ? 9.5f : 8.5f)){
                return Action::Double;
            }
            // 2-deck 65% pen: 10v11 Double TC >= 8.0, 6-deck 80% pen: TC >= 9.5
            if (dealerValue == 11 && trueCount >= (useSixDeck ? 9.5f : 8.0f)){
                return Action::Double;
            }
            break;

        case 9:
            // 2-deck 65% pen: 9v2 Double TC >= 2.5, 6-deck 80% pen: TC >= 2.5
            if (dealerValue == 2 && trueCount >= 2.5f){
                return Action::Double;
            }
            // 2-deck 65% pen: 9v7 Double TC >= 11.0, 6-deck 80% pen: TC >= 10.0
            if (dealerValue == 7 && trueCount >= (useSixDeck ? 10.0f : 11.0f)){
                return Action::Double;
            }
            break;
...
Action RAPCStrategy::shouldDeviatefromSplit(Rank playerRank, Rank dealerUpcard, float trueCount){
    ...
    const bool useSixDeck = initial_decks >= 5.5f;
    switch (playerValue) {
        // 2-deck 65% pen: Split 10s v5 TC >= 14.0, 6-deck 80% pen: TC >= 15.5
        // 2-deck 65% pen: Split 10s v6 TC >= 14.0, 6-deck 80% pen: TC >= 13.5
        case 10: 
            if (dealerValue == 5 && trueCount >= (useSixDeck ? 15.5f : 14.0f)) {
                return Action::Split;
            }
            if (dealerValue == 6 && trueCount >= (useSixDeck ? 13.5f : 14.0f)) {
                return Action::Split;
            }
            break;
...
Action RAPCStrategy::shouldSurrender(int playerTotal, Rank dealerUpcard, float trueCount){
    ...
    switch (playerTotal) {
        case 16:
            // 2-deck 65% pen: 16v9 Surrender TC >= 1.5, 6-deck 80% pen: TC >= -2.0
            if (dealerValue == 9 && trueCount >= (useSixDeck ? -2.0f : 1.5f)) {
                return Action::Surrender;
            }
            // 2-deck 65% pen: 16v10 Surrender TC >= -8.0, 6-deck 80% pen: TC >= -11.0
            if (dealerValue == 10 && trueCount >= (useSixDeck ? -11.0f : -8.0f)) {
                return Action::Surrender;
            }
            // 2-deck 65% pen: 16v11 Surrender TC >= -6.0, 6-deck 80% pen: TC >= -6.0
            if (dealerValue == 11 && trueCount >= -6.0f) {
                return Action::Surrender;
            }
            break;
        case 15:
            // 2-deck 65% pen: 15v9 Surrender TC >= 7.0, 6-deck 80% pen: TC >= 7.0
            if (dealerValue == 9 && trueCount >= 7.0f) {
                return Action::Surrender;
            }
            // 2-deck 65% pen: 15v10 Surrender TC >= -0.5, 6-deck 80% pen: TC >= -1.5
            if (dealerValue == 10 && trueCount >= (useSixDeck ? -1.5f : -0.5f)) {
                return Action::Surrender;
            }
            // 2-deck 65% pen: 15v11 Surrender TC >= 3.0, 6-deck 80% pen: TC >= 4.5
            if (dealerValue == 11 && trueCount >= (useSixDeck ? 4.5f : 3.0f)) {
                return Action::Surrender;
            }
            break;
        case 14:
            // 2-deck 65% pen: 14v10 Surrender TC >= 8.0, 6-deck 80% pen: TC >= 8.0
            if (dealerValue == 10 && trueCount >= 8.0f) {
                return Action::Surrender;
            }
            break;
        default: return Action::Skip; break;
